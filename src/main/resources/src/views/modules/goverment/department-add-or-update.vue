    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.deptId) {
            this.$http({
              url: this.$http.adornUrl(`/goverment/department/info/${this.dataForm.deptId}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.deptName = data.department.deptName
                this.dataForm.location = data.department.location
                this.dataForm.workTime = data.department.workTime
                this.dataForm.description = data.department.description
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/goverment/department/${!this.dataForm.deptId ? 'save' : 'update'}`),
              method: 'post',
